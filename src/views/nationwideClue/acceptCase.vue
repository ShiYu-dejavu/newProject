      ...
      <el-col :span="16">
        <el-card  class="card-container">
          ...
          <table class="tb-content">
            ...
            <tr>
              <td class="tb-title">实际用工所在地</td>
              <td>{{dataForm.ACTUAL_WORK_PLACE}}</td>
              <td class="tb-title">反映人姓名</td>
              <td>{{dataForm.REFLECT_PERSON_NAME}}</td>
            </tr>
            <tr>
              <td class="tb-title">涉案项目（单位）</td>
              <td colspan="3" v-if="dataForm.INDUSTRY_FIELD_NUM=='1'">{{dataForm.BACKPAY_PROJECT}}</td>
              <td colspan="3" v-else>{{dataForm.BE_REPORTED_COMPANY}}</td>
            </tr>
            <tr>
              <td class="tb-title">基本情况摘要</td>
              <td colspan="6">{{dataForm.BACK_PAY_REMARK}}</td>
            </tr>
          </table>
          <div class="form-box">
            <el-form ref="form" :model="form" label-width="100px" :rules="formRules">
              ...
              <div v-if="form.status == 2 || form.status == 1">
                <el-row>
                  <el-col :span='12'>
                    <el-form-item label="受理人：" prop="audit_person">
                  ...
                  <el-col :span='12'>
                    <el-form-item label="受理时间：" prop="deal_time">
                      <el-date-picker style="width:100%" v-model="form.deal_time" value-format="yyyy-MM-dd" type="date"
                        placeholder="请选择受理时间" clearable size='small'>
                      </el-date-picker>
                    </el-form-item>
                  </el-col>
                </el-row>
              ...
              <div v-else>
                 <span class="fl mr-10 ">案件查询</span>
                  <div class="w-200 fl mr-10 mb">
                    <el-input v-model="searchList.case_number" clearable size="small" placeholder="请输入案件编号"></el-input>
                  </div>
                  <div class="w-200 fl mr-10 mb">
...
import * as api from "@/api/backpay";
  export default {
    ...
    data() {
      return {
        ...
        formRules:{
          ...
          audit_person:[{required: true, message: '请选择受理人', trigger: 'change'}],
          deal_time:[{required: true, message: '请选择受理时间', trigger: 'change'}],
          reason:[{}],
        },
        options: [],
        showAll: false,
    ...
    methods: {
      ...
      //取消
      cancel(){
        this.$router.push('/nationwideClue/index')
      },
      ...
      getAllMsg(){
        ...
        let data = [];
        data.clue_code = this.$route.query.clue_code; //线索编号
        api
          .getallmsg(data)
          .then(res => {
      ...
      saveAuth(){
        ...
        if(this.form.status=='1'){ //受理
            ...
            this.form.clue_code = this.dataForm.CLUE_CODE;
            api
            .saveAuthMsg(this.form)
            .then(res => {
              if (res.data.code === 201) {
                this.$message({
                  type: "success",
            ...
            data.type_code = this.dataForm.CLUE_CODE;
            api
            .editBookInfo(data)
            .then(res => {
      ...
      mergeCase(caseNum){
        this.$confirm(`是否将此举报投诉并案到编号${caseNum}的的案件中?`, '提示', {
          ...
          type: 'warning'
            ...
            data.clue_code = this.dataForm.CLUE_CODE;
            api
              .saveAuthMsg(data)
              .then(res => {
                if (res.data.code === 201) {
